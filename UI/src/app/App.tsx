...
import {
  ...
  Server, Users, Clock, CheckCircle, Search,
...
function Sidebar({ activeSection, setActiveSection, isDark, isOpen, setIsOpen }: SidebarProps) {
  return (
    <aside
    ...
    >
      ...
      <nav className="flex-1 px-3 py-6 space-y-1.5 overflow-y-auto">
        {isOpen && (
          <p className={`px-3 text-[10px] font-body font-bold uppercase tracking-widest mb-3 ${isDark ? "text-slate-500" : "text-slate-400"}`}>
            Menu Utama
          </p>
        )}
        {NAV_ITEMS.map((item) => {
      ...
      {isOpen && (
        <div className={`p-4 m-3 rounded-2xl border transition-all ${
          isDark ? "border-slate-800/80 bg-slate-900/40" : "border-slate-200/80 bg-slate-50"
        }`}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className={`text-xs font-heading font-bold truncate ${isDark ? "text-slate-200" : "text-slate-800"}`}>BKPSDM Kab. PPU</p>
              <p className={`text-[10px] font-body truncate ${isDark ? "text-slate-400" : "text-slate-500"}`}>Penajam Paser Utara</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={loading}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-body font-bold transition-all shadow-md active:scale-95 disabled:opacity-50"
            >
              {loading ? "Cek..." : "Cek NIP"}
            </button>
          </div>
          ...
          <div>
            <p className={`text-[11px] font-body font-medium mb-2 ${isDark ? "text-slate-400" : "text-slate-500"}`}>Contoh NIP Pegawai PPU:</p>
          ...
          {result ? (
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b pb-2.5 border-slate-200 dark:border-slate-800">
                <span className="text-xs font-body font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
              ...
              <div>
                <p className={`text-[10px] font-body uppercase font-bold tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>Nama Pegawai (Sensored)</p>
                <p className={`text-sm font-heading font-bold mt-0.5 ${isDark ? "text-white" : "text-slate-900"}`}>{maskNama(result.nama)}</p>
              </div>

              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  <p className={`text-[10px] font-body uppercase font-bold tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>NIP (Sensored)</p>
                  <p className="text-xs font-mono font-bold text-blue-600 dark:text-blue-400 mt-0.5">{maskNip(result.nip)}</p>
                </div>
                <div>
                  <p className={`text-[10px] font-body uppercase font-bold tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>Golongan</p>
                  <p className="text-xs font-stat font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">{result.gol}</p>
                </div>
                <div className="col-span-2">
                  <p className={`text-[10px] font-body uppercase font-bold tracking-wider ${isDark ? "text-slate-400" : "text-slate-500"}`}>Jabatan & Perangkat Daerah</p>
                  <p className={`text-xs mt-0.5 font-body font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-8 max-w-7xl">
          ...
          {(activeSection === "Beranda Informasi" || activeSection === "Statistik ASN PPU") && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              ...
              >
                <h3 className={`text-base font-heading font-bold mb-1 ${isDark ? "text-white" : "text-slate-900"}`}>
                  Aktivitas Penggunaan Layanan PPU
                </h3>
                <p className={`text-xs font-body mb-6 ${isDark ? "text-slate-400" : "text-slate-600"}`}>Grafik aktivitas layanan harian pegawai</p>
              ...
              >
                <div>
                  ...
                  <p className={`text-xs font-body mb-4 ${isDark ? "text-slate-400" : "text-slate-600"}`}>Persentase di OPD Utama</p>
                ...
                <div className="space-y-1.5 mt-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                  {OPD_DISTRIBUTION.map((d, i) => (
                    <div key={d.name} className="flex items-center justify-between text-xs font-body font-semibold">
                      ...
                      <span className="text-blue-600 dark:text-blue-400 font-stat font-bold">{d.percent}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {(activeSection === "Beranda Informasi" || activeSection === "Unduhan & Dataset") && (
            <div className="space-y-4">
              <div>
                <h3 className={`text-base font-heading font-bold ${isDark ? "text-white" : "text-slate-900"}`}>
          ...
          {/* Footer */}
          <footer className={`pt-6 pb-6 border-t flex items-center justify-between text-xs font-body ${
